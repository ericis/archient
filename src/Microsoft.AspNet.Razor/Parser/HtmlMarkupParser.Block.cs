...
namespace Microsoft.AspNet.Razor.Parser
{
    public partial class HtmlMarkupParser
    {
        ...
        private bool RestOfTag(Tuple<HtmlSymbol, SourceLocation> tag, Stack<Tuple<HtmlSymbol, SourceLocation>> tags)
        {
            ...
            if (!seenClose)
            {
                Context.OnError(tag.Item2, string.Format(RazorResources.ParseError_UnfinishedTag,tag.Item1.Content));
            }
            else
            {
                if (!isEmpty)
            ...
            while (!seenEndScript && !EndOfFile)
            {
                ...
                if (Optional(HtmlSymbolType.Solidus))
                {
                    ...
                    if (At(HtmlSymbolType.Text) && String.Equals(CurrentSymbol.Content, "script", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (!Optional(HtmlSymbolType.CloseAngle))
                        {
                            Context.OnError(tagStart, string.Format(RazorResources.ParseError_UnfinishedTag, "script"));
                        }
                        seenEndScript = true;
                    }
                }
        ...
        private bool RemoveTag(Stack<Tuple<HtmlSymbol, SourceLocation>> tags, string tagName, SourceLocation tagStart)
        {
            ...
            if (currentTag != null)
            {
                Context.OnError(currentTag.Item2, string.Format(RazorResources.ParseError_MissingEndTag, currentTag.Item1.Content));
            }
            else
            {
                Context.OnError(tagStart, string.Format(RazorResources.ParseError_UnexpectedEndTag,tagName));
            }
            return false;
        }
        ...
        private void EndTagBlock(Stack<Tuple<HtmlSymbol, SourceLocation>> tags, bool complete)
        {
            if (tags.Count > 0)
            {
                ...
                Context.OnError(tag.Item2, string.Format(RazorResources.ParseError_MissingEndTag, tag.Item1.Content));
            }
            else if (complete)
            {
                Span.EditHandler.AcceptedCharacters = AcceptedCharacters.None;
