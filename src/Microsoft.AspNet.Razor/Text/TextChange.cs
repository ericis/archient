...
namespace Microsoft.AspNet.Razor.Text
{
    public struct TextChange
    {
            ...
            : this()
        {
            if (oldPosition < 0)
            {
                throw new ArgumentOutOfRangeException("oldPosition", string.Format(CommonResources.Argument_Must_Be_GreaterThanOrEqualTo, 0));
            }
            if (newPosition < 0)
            {
                throw new ArgumentOutOfRangeException("newPosition", string.Format(CommonResources.Argument_Must_Be_GreaterThanOrEqualTo, 0));
            }
            if (oldLength < 0)
            {
                throw new ArgumentOutOfRangeException("oldLength", string.Format(CommonResources.Argument_Must_Be_GreaterThanOrEqualTo, 0));
            }
            if (newLength < 0)
            {
                throw new ArgumentOutOfRangeException("newLength", string.Format(CommonResources.Argument_Must_Be_GreaterThanOrEqualTo, 0));
            }
            if (oldBuffer == null)
            {
