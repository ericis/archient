...
namespace Microsoft.AspNet.Razor.Editor
{
    internal class BackgroundParser : IDisposable
    {
        ...
        private class MainThreadState : ThreadStateBase, IDisposable
        {
            ...
            public void QueueChange(TextChange change)
            {
                RazorEditorTrace.TraceLine(string.Format(RazorResources.Trace_QueuingParse,Path.GetFileName(_fileName), change));
                EnsureOnThread();
                lock (_stateLock)
                {
        ...
        private class BackgroundThread : ThreadStateBase
        {
            ...
            private void WorkerLoop()
            {
                ...
                try
                {
                    RazorEditorTrace.TraceLine(string.Format(RazorResources.Trace_BackgroundThreadStart,fileNameOnly));
                    EnsureOnThread();
                    ...
                    while (!_shutdownToken.IsCancellationRequested)
                    {
                        ...
                        {
                            RazorEditorTrace.TraceLine(string.Format(RazorResources.Trace_ChangesArrived,fileNameOnly, parcel.Changes.Count));
                            try
                            {
                                ...
                                using (var linkedCancel = CancellationTokenSource.CreateLinkedTokenSource(_shutdownToken, parcel.CancelToken))
                                {
                                    if (!linkedCancel.IsCancellationRequested)
                                    {
                                        ...
                                        RazorEditorTrace.TraceLine(
                                            string.Format(RazorResources.Trace_ParseComplete,
                                            fileNameOnly,
                                        ...
                                        if (results != null && !linkedCancel.IsCancellationRequested)
                                        {
                                            ...
                                            RazorEditorTrace.TraceLine(string.Format(RazorResources.Trace_TreesCompared,
                                                fileNameOnly,
                                                elapsedMs.HasValue ? elapsedMs.Value.ToString(CultureInfo.InvariantCulture) : "?",
                                        ...
                                        else
                                        {
                                            ...
                                            RazorEditorTrace.TraceLine(string.Format(RazorResources.Trace_ChangesDiscarded, fileNameOnly, allChanges.Count));
                        ...
                        {
                            RazorEditorTrace.TraceLine(string.Format(RazorResources.Trace_NoChangesArrived,fileNameOnly));
#if NET45
                            // No Yield in CoreCLR
                ...
                {
                    RazorEditorTrace.TraceLine(string.Format(RazorResources.Trace_BackgroundThreadShutdown,fileNameOnly));
